...
// process.container.pod_uid and process.cgroup.path, and that both fields are
// serialized in CWS events.
func TestCGroupKubernetesPodUID(t *testing.T) {
	...
	ruleDefs := []*rules.RuleDefinition{
		{
			...
			Expression: `open.file.path == "{{.Root}}/test-kubernetes-pod-uid" && process.container.pod_uid == "` + podUID + `" && process.cgroup.path =~ "*pod` + systemdPodUID + `.slice"`,
		},
	}
	test, err := newTestModule(t, nil, ruleDefs)
	if err != nil {
	...
	test.WaitSignalFromRule(t, func() error {
		...
		assertFieldContains(t, event, "process.cgroup.path", "pod"+systemdPodUID+".slice")

		test.validateOpenSchema(t, event)
		validateSerializedKubernetesPodCGroup(t, test, event, podUID, "pod"+systemdPodUID+".slice")
	}, "test_cgroup_kubernetes_pod_uid")
}

func validateSerializedKubernetesPodCGroup(t *testing.T, test *testModule, event *model.Event, podUID string, cgroupPathFragment string) {
	t.Helper()
	...
	type serializedCGroup struct {
		Path string `json:"path"`
	}
	var payload struct {
		Container *serializedContainer `json:"container"`
		CGroup    *serializedCGroup    `json:"cgroup"`
		Process   struct {
			Container *serializedContainer `json:"container"`
			CGroup    *serializedCGroup    `json:"cgroup"`
		} `json:"process"`
	...
	require.NotNil(t, payload.CGroup)
	assert.Contains(t, payload.CGroup.Path, cgroupPathFragment)
	require.NotNil(t, payload.Process.Container)
	assert.Equal(t, podUID, payload.Process.Container.PodUID)
	require.NotNil(t, payload.Process.CGroup)
	assert.Contains(t, payload.Process.CGroup.Path, cgroupPathFragment)
}
